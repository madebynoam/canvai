...
/** V4 Token overview — Signal red + elevated canvas + dial tokens */
export function Tokens() {
  ...
  const dial = [
    { name: '--dial-track',       value: 'oklch(0.900 0.003 80)' },
    { name: '--dial-thumb',       value: 'oklch(0.300 0.005 80)' },
    { name: '--dial-tick',        value: 'oklch(0.800 0.003 80)' },
    { name: '--dial-tick-active', value: 'oklch(0.300 0.005 80)' },
  ]
  ...
  return (
    <div style={{ padding: 32, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif' }}>
      <h2 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 24 }}>
        V4 Tokens — Dial Track + Signal Red
      </h2>
      ...
      <Label sub="Signal red h=28">Accent</Label>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 24, marginTop: 8 }}>
        {accent.map(t => (
          <Swatch key={t.name} color={t.value} label={t.name} value={t.value} />
        ))}
      ...
      <Label sub="V4-specific iteration dial track">Dial</Label>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 24, marginTop: 8 }}>
        {dial.map(t => (
          <Swatch key={t.name} color={t.value} label={t.name} value={t.value} />
        ))}
