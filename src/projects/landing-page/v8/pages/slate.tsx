...
// ── Slate — Codex Dark Mode (V8 Context) ──────────────────────────────────────
// Based on Codex context: blue gradients but dark theme. Same tabbed layout,
// card-based features, centered hero. Dark slate background.

const C = {
  bg: 'oklch(0.140 0.015 250)',
  bgGradient1: 'oklch(0.250 0.100 250)',
  bgGradient2: 'oklch(0.200 0.120 280)',
  bgCard: 'oklch(0.180 0.020 250)',
  bgCardHover: 'oklch(0.200 0.025 250)',
  text: 'oklch(0.950 0.010 250)',
  textSec: 'oklch(0.700 0.015 250)',
  border: 'oklch(0.250 0.020 250)',
...
export function Slate() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100%', overflow: 'auto', fontFamily: font, WebkitFontSmoothing: 'antialiased' }}>
      {/* Hero with gradient */}
      <div style={{
        background: `linear-gradient(180deg, ${C.bgGradient1} 0%, ${C.bgGradient2} 40%, ${C.bg} 100%)`,
        paddingBottom: 80,
      }}>
        {/* Nav */}
        <nav style={{
          ...
          <div style={{
            ...
            color: C.bg, fontWeight: 700, fontSize: 14,
          }}>B</div>
          <div style={{ display: 'flex', gap: 32 }}>
            {['Product', 'Docs', 'Pricing'].map(item => (
              <span key={item} style={{ fontSize: 14, color: C.textSec, cursor: 'default' }}>{item}</span>
            ))}
          </div>
          <button {...bhB} style={{
          ...
          {tabs.map(t => (
            <button
              ...
              style={{
                ...
                color: activeTab === t.id ? C.bg : C.textSec,
      ...
      <section style={{ padding: '0 64px', maxWidth: 1000, margin: '-40px auto 0' }}>
        <div {...ihB} style={{
          ...
          boxShadow: ih ? '0 24px 64px oklch(0 0 0 / 0.4)' : '0 12px 40px oklch(0 0 0 / 0.2)',
