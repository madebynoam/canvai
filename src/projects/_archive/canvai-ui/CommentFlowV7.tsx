...
import { MessageSquare, Send, Check, X, MoreHorizontal, Pencil, Trash2, Link, ExternalLink, Copy, Github } from 'lucide-react'
import { ACCENT, SURFACE, SURFACE_SUBTLE, BORDER, TEXT, TEXT_SEC, TEXT_TER, FONT, CANVAS, MONO } from './tokens'
import { SPRING, useSpring } from './spring'

/* ── Constants ────────────────────────────── */

const PURPLE = '#6366F1'
const PURPLE_MUTED = 'rgba(99, 102, 241, 0.12)'
const GREEN = '#059669'
...
const THREAD_MENU_ITEMS = [
  { label: 'Copy link', icon: <Copy size={16} strokeWidth={1.5} /> },
  { label: 'Delete', icon: <Trash2 size={16} strokeWidth={1.5} />, destructive: true, separator: true },
]

/* ── Primitives (matching V5 SystemCompositions exactly) ── */

function Avatar({ name, size = 28, color }: { name: string; size?: number; color?: string }) {
  const colors: Record<string, string> = { G: PURPLE, N: ACCENT, S: '#0EA5E9', M: '#8B5CF6', A: '#10B981' }
  const letter = name.charAt(0).toUpperCase()
  return (
    <div style={{
      width: size, height: size, borderRadius: '50%', flexShrink: 0,
      backgroundColor: color || colors[letter] || TEXT_SEC, color: '#fff',
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: '2px 8px', borderRadius: 12, border: `1px solid ${active ? PURPLE : hovered ? '#D1D5DB' : BORDER}`,
        backgroundColor: active ? PURPLE_MUTED : hovered ? '#F3F4F6' : '#FAFAFA',
        fontSize: 11, color: active ? PURPLE : TEXT_SEC, fontWeight: 500, fontFamily: FONT,
...
function StepBadge({ step }: { step: number }) {
  return (
    <div style={{
      width: 20, height: 20, borderRadius: '50%', backgroundColor: PURPLE,
...
function MiniCanvas({ children, style }: { children?: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <div style={{
      width: '100%', height: 160, backgroundColor: CANVAS, borderRadius: 8,
      ...
      <div style={{
        ...
        width: 96, height: 56, borderRadius: 4,
        backgroundColor: SURFACE, border: `1px solid ${BORDER}`,
      }} />
      {children}
    </div>
...
export function FlowStep1_SignIn() {
  ...
  useEffect(() => {
    if ((phase === 'auth' || phase === 'code' || phase === 'done') && cardRef.current) {
      ...
      cardSpring.set(1, (v) => {
        if (cardRef.current) {
          cardRef.current.style.transform = `scale(${v}) translateY(${(1 - v) * 8}px)`
  ...
  return (
    <div style={{ fontFamily: FONT, display: 'flex', flexDirection: 'column', gap: 20, padding: 8 }}>
      ...
      <div style={{
        width: 380, minHeight: 300, borderRadius: 10, backgroundColor: CANVAS,
        border: `1px solid ${BORDER}`, position: 'relative', overflow: 'hidden',
      }}>
        {/* Fake frames */}
        <div style={{
          position: 'absolute', top: 40, left: 40,
          width: 120, height: 72, borderRadius: 4,
          backgroundColor: SURFACE, border: `1px solid ${BORDER}`,
        }} />
        <div style={{
        ...
        {phase !== 'idle' && (
          <div ref={cardRef} style={{
            ...
            {phase === 'auth' && (
              <>
                <Github size={24} strokeWidth={1.5} color={TEXT} />
                <div style={{ fontSize: 13, fontWeight: 600, color: TEXT, textAlign: 'center' }}>
                  Sign in with GitHub
                </div>
                <div style={{ fontSize: 12, color: TEXT_SEC, textAlign: 'center', lineHeight: 1.5, textWrap: 'pretty' } as React.CSSProperties}>
            ...
            {phase === 'code' && (
              <>
                <div style={{ fontSize: 12, color: TEXT_SEC, textAlign: 'center', lineHeight: 1.5, textWrap: 'pretty' } as React.CSSProperties}>
                  Enter this code at <span style={{ fontWeight: 600, color: TEXT }}>github.com/login/device</span>
                </div>
                <div style={{
                  padding: '12px 24px', borderRadius: 8,
                  backgroundColor: CANVAS, border: `1px solid ${BORDER}`,
                  fontFamily: MONO, fontSize: 20, fontWeight: 700, color: TEXT,
                ...
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: SURFACE, fontSize: 12, color: TEXT_SEC,
            ...
            {phase === 'done' && (
              <>
                <div style={{
                  width: 32, height: 32, borderRadius: '50%', backgroundColor: GREEN_BG,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  <Check size={16} strokeWidth={2} color={GREEN} />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Avatar name="Noam" size={32} />
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: TEXT }}>Noam</div>
                    <div style={{ fontSize: 11, color: TEXT_TER }}>@madebynoam</div>
                  </div>
                </div>
                <div style={{ fontSize: 11, color: TEXT_TER, textAlign: 'center', lineHeight: 1.4, textWrap: 'pretty' } as React.CSSProperties}>
      ...
      <Note>
        Click the purple FAB to start the sign-in flow. Three states: auth prompt → device code → signed in. Avatar badge appears on the FAB after auth.
      </Note>
    </div>
  )
}
...
export function FlowStep2_Compose() {
  ...
  return (
    <div style={{ fontFamily: FONT, display: 'flex', flexDirection: 'column', gap: 20, padding: 8 }}>
      ...
      <div style={{
        width: 380, minHeight: 340, borderRadius: 10, backgroundColor: CANVAS,
        border: `1px solid ${BORDER}`, position: 'relative', overflow: 'hidden',
      }}>
        {/* Target element */}
        <div
          ...
          style={{
            ...
            border: phase === 'targeting' ? `2px solid ${PURPLE}` : `1px solid ${BORDER}`,
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4,
            cursor: phase === 'targeting' ? 'crosshair' : 'default',
          }}>
          <div style={{ fontSize: 11, fontWeight: 500, color: TEXT }}>PricingCard</div>
          <div style={{ fontSize: 10, color: TEXT_TER }}>
        ...
        {phase === 'targeting' && (
          <div style={{
            ...
            width: 204, height: 104, borderRadius: 10,
        ...
        {phase === 'compose' && (
          <div ref={cardRef} style={{
            ...
            width: 280, backgroundColor: SURFACE, borderRadius: 10, padding: 12,
            border: `1px solid ${BORDER}`,
            boxShadow: '0 4px 24px rgba(0,0,0,0.08), 0 1px 4px rgba(0,0,0,0.04)',
            display: 'flex', flexDirection: 'column', gap: 8,
            transform: 'scale(0.96)', opacity: 0,
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
              ...
              <textarea
                ...
                style={{
                  flex: 1, minHeight: 56, padding: '8px 10px', borderRadius: 6,
                  backgroundColor: SURFACE_SUBTLE, border: `1px solid ${BORDER}`,
                  fontSize: 12, color: text ? TEXT : TEXT_TER, fontFamily: FONT,
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              ...
              <button style={{
                ...
                backgroundColor: text.trim() ? PURPLE : 'rgba(99,102,241,0.15)',
                color: text.trim() ? '#fff' : TEXT_TER,
        ...
        <button
          ...
          style={{
            position: 'absolute', bottom: 16, right: 16,
            width: 40, height: 40, borderRadius: '50%', border: 'none',
            backgroundColor: phase === 'targeting' ? TEXT : PURPLE, color: '#fff',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            boxShadow: `0 2px 8px ${phase === 'targeting' ? 'rgba(0,0,0,0.2)' : 'rgba(99,102,241,0.3)'}`,
            cursor: 'default',
          }}>
          {phase === 'targeting' ? <X size={16} strokeWidth={1.5} /> : <MessageSquare size={16} strokeWidth={1.5} />}
        </button>
      </div>

      <Note>
        Click the purple FAB to enter targeting mode. Purple highlight follows cursor. Click an element to open the compose card. Textarea is typeable. Post activates when text is entered.
      </Note>
    </div>
  )
}
...
/* ═══════════════════════════════════════════════════════
   Step 3: Thread — full card (matching V5 exactly)
   ═══════════════════════════════════════════════════════ */
...
export function FlowStep3_Thread() {
  ...
  return (
    <div style={{ fontFamily: FONT, display: 'flex', flexDirection: 'column', gap: 20, padding: 8 }}>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              backgroundColor: replyText.trim() ? PURPLE : '#F3F4F6',
      ...
      <Note>
        Hover messages to see quick reactions (👍 🤔 🔥) and ⋯ menu. Menus open with spring scaleY. Reply input is typeable — Enter or click Send. Send has spring press animation. Thread menu has Copy link + Delete.
      </Note>
    </div>
  )
}
  ...
  return (
    <div style={{ fontFamily: FONT, display: 'flex', flexDirection: 'column', gap: 20, padding: 8 }}>
      ...
      <MiniCanvas style={{ width: 380, height: 200 }}>
        ...
        <div style={{ position: 'absolute', top: 88, left: 260 }}>
          ...
          <div style={{
            ...
            backgroundColor: ACCENT, border: `2px solid ${CANVAS}`,
          }} />
        </div>
      ...
      <div style={{
        ...
        backgroundColor: SURFACE_SUBTLE, borderRadius: 8, border: `1px solid ${BORDER}`,
      }}>
        <LegendRow>
        ...
        <LegendRow>
          <div style={{ position: 'relative' }}>
            ...
            <div style={{
              ...
              minWidth: 12, height: 12, borderRadius: 6, backgroundColor: PURPLE,
              color: '#fff', fontSize: 7, fontWeight: 700, fontFamily: FONT,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              padding: '0 2px', border: `1.5px solid ${SURFACE_SUBTLE}`,
            }}>3</div>
          </div>
        ...
        <LegendRow>
          <div style={{ position: 'relative' }}>
            ...
            <div style={{
              ...
              backgroundColor: ACCENT, border: `1.5px solid ${SURFACE_SUBTLE}`,
            }} />
          </div>
        ...
        <LegendRow>
          <div style={{ opacity: 0.5 }}>
            <div style={{
              width: 18, height: 18, borderRadius: '50%', backgroundColor: '#D1D5DB',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
      ...
      <Note>
        Pins track element positions via rAF (same pattern as annotation markers). Reply count badge top-right. Orange dot bottom-right means an annotation was promoted from this thread. Resolved pins fade to 50% opacity.
      </Note>
    </div>
  )
}
...
function LegendRow({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10, color: TEXT_SEC }}>
      {children}
    </div>
  )
}
    ...
    <div style={{ fontFamily: FONT, display: 'flex', flexDirection: 'column', gap: 20, padding: 8 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: TEXT }}>Comment</span>
            <span style={{ fontSize: 11, color: TEXT_TER }}>1 reply</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <HoverButton title="Resolve thread"><Check size={14} strokeWidth={1.5} /></HoverButton>
            <HoverButton title="Close panel"><X size={14} strokeWidth={1.5} /></HoverButton>
          </div>
        </div>

        <div style={{ fontSize: 11, color: TEXT_TER, letterSpacing: '0.02em', padding: '4px 0', borderBottom: `1px solid ${BORDER}` }}>
          PricingCard &middot; h2
        </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: TEXT }}>Gustav</span>
              <span style={{ fontSize: 11, color: TEXT_TER }}>5 min ago</span>
              <div style={{ flex: 1 }} />
        ...
        {promoted && (
          <div ref={bannerRef} style={{
            ...
            <Pencil size={12} strokeWidth={1.5} color={ACCENT} />
...
export function FlowStep6_DeepLink() {
  ...
  const handleCopy = useCallback(() => {
    ...
    if (toastRef.current) {
      ...
      toastSpring.set(1, (v) => {
        if (toastRef.current) {
          toastRef.current.style.transform = `translateY(${(1 - v) * 16}px)`
  ...
  return (
    <div style={{ fontFamily: FONT, display: 'flex', flexDirection: 'column', gap: 20, padding: 8 }}>
      ...
      {/* URL bar */}
      <div style={{
        padding: '12px 16px', borderRadius: 10, backgroundColor: SURFACE,
        border: `1px solid ${BORDER}`,
        boxShadow: '0 2px 8px rgba(0,0,0,0.04)',
        display: 'flex', alignItems: 'center', gap: 8,
      }}>
        <ExternalLink size={14} strokeWidth={1.5} color={TEXT_TER} />
        <span style={{ fontFamily: MONO, fontSize: 12, color: TEXT_SEC }}>
          your-team.github.io/design
        </span>
        <span style={{ fontFamily: MONO, fontSize: 12, color: PURPLE, fontWeight: 600 }}>
          ?comment=42
        </span>
        <div style={{ flex: 1 }} />
        <button
          ...
          style={{
            ...
            backgroundColor: SURFACE, fontSize: 11, fontWeight: 500,
      ...
      <div style={{
        display: 'flex', alignItems: 'center', gap: 8,
        padding: 16, borderRadius: 10, backgroundColor: SURFACE_SUBTLE,
        border: `1px solid ${BORDER}`,
      }}>
...
function FlowChip({ label, highlight }: { label: string; highlight?: boolean }) {
  return (
    <div style={{
      ...
      backgroundColor: highlight ? PURPLE_MUTED : SURFACE,
      border: `1px solid ${highlight ? 'rgba(99,102,241,0.2)' : BORDER}`,
...
function FlowArrow() {
  return (
    <svg width="16" height="12" viewBox="0 0 16 12" fill="none" style={{ flexShrink: 0 }}>
      <path d="M1 6 L11 6 M8 2 L13 6 L8 10" stroke={TEXT_TER} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
