...
import { N, A, S, R, T, ICON, FONT, F } from '../tokens'
...
/* ─── useDropdownSpring — same pattern as ProjectPicker ─ */

function useDropdownSpring(open: boolean) {
...
export function AnnotationBadge({ count, onClick }: { count: number; onClick: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: R.control,
        backgroundColor: hovered && !isResolved ? 'rgba(255,255,255,0.06)' : 'transparent',
...
export function AnnotationDropdown({
  annotations,
  open,
  dropdownRef,
  onApplyAll,
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: `0 ${S.xs}px ${S.lg}px rgba(0, 0, 0, 0.24), 0 1px ${S.xs}px rgba(0, 0, 0, 0.12)`,
        padding: 0,
        transformOrigin: 'top right',
        transform: forceOpen ? 'scale(1) translateY(0)' : `scale(0.92) translateY(-${S.xs}px)`,
...
export function useAnnotationPanel(initialAnnotations: Annotation[]) {
  const [open, setOpen] = useState(false)
  const [annotations, setAnnotations] = useState<Annotation[]>(initialAnnotations)
  const containerRef = useRef<HTMLDivElement>(null)
  const dropdownRef = useDropdownSpring(open)
  ...
  return { open, setOpen, annotations, drafts, containerRef, dropdownRef, handleApplyAll, handleApplyOne }
}
