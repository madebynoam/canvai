    ...
    <div style={{ padding: S.xxl, fontFamily: FONT }}>
      ...
      <Section title="Neutrals — white chrome experiment">
        <div style={{ display: 'flex', flexDirection: 'column', gap: S.sm }}>
          <TokenSwatch color="var(--chrome)" label="chrome" sublabel="L=1.000 — white sidebar, topbar" oklch={{ l: 1.000, c: 0, h: 0 }} tokenPath="--chrome" />
          ...
          <TokenSwatch color="var(--canvas)" label="canvas" sublabel="L=0.972 — workspace" oklch={{ l: 0.972, c: 0.003, h: 80 }} tokenPath="--canvas" />
      ...
      <Section title="Accent — charcoal (h=80, c=0.005)">
        <div style={{ display: 'flex', flexDirection: 'column', gap: S.sm }}>
          <TokenSwatch color="var(--accent)" label="accent" sublabel="L=0.30 — primary action" oklch={{ l: 0.300, c: 0.005, h: 80 }} tokenPath="--accent" />
          <TokenSwatch color="var(--accent-hover)" label="hover" sublabel="L=0.40 — hover state" oklch={{ l: 0.400, c: 0.005, h: 80 }} tokenPath="--accent-hover" />
          <TokenSwatch color="var(--accent-muted)" label="muted" sublabel="L=0.92 — subtle bg" oklch={{ l: 0.920, c: 0.003, h: 80 }} tokenPath="--accent-muted" />
          <TokenSwatch color="var(--accent-strong)" label="strong" sublabel="L=0.22 — pressed state" oklch={{ l: 0.220, c: 0.005, h: 80 }} tokenPath="--accent-strong" />
          <TokenSwatch color="var(--accent-border)" label="border" sublabel="L=0.70 — accent borders" oklch={{ l: 0.700, c: 0.005, h: 80 }} tokenPath="--accent-border" />
        </div>
      </Section>
