...
import { Avatar, HoverButton, Label } from '../components'

/** Same shell assembly as V1 — warm neutrals + Braun green accent via token overrides. */
export function ShellRams() {
  const [expanded, setExpanded] = useState<Record<string, boolean>>({ 'V1': true })

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 8 }}>
      <Label sub="Warm neutrals (h=80, c=0.003) + Braun green accent for functional indicators only.">Shell — Rams Palette</Label>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }} />
          {/* Functional indicator — the only color */}
          <div style={{
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            {[
              ...
              { name: 'V2', pages: ['Shell Rams', 'Rams Palette'] },
            ].map((iter, i) => (
              <div key={iter.name} style={{
                ...
                {expanded[iter.name] && iter.pages.map((page, pi) => (
                  <div key={page} style={{
                    ...
                    color: pi === 0 && iter.name === 'V2' ? 'var(--text-primary)' : 'var(--text-secondary)',
                    fontWeight: pi === 0 && iter.name === 'V2' ? 600 : 400,
                    backgroundColor: pi === 0 && iter.name === 'V2' ? 'var(--chrome-active)' : 'transparent',
